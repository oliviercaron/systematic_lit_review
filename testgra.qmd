...
def create_graph(dataframe, start_year, end_year):
    ...
    # Loop through each node in the graph to collect its attributes
    rows = []
    for node in G.nodes():
        # Initialize a dictionary to store attributes of the current node
        node_info = {'Node': node}
        
        # Add other attributes
        for col in columns_to_collect:
            node_info[col] = G.nodes[node].get(col, None)  # Use None if the attribute is not set
...
# Example usage
G_2015_2019, df_2015_2019 = create_graph(list_articles, 2013, 2017)
