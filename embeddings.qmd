...
## A first Word2Vec embeddings analysis

```{r}
#| label: word2vec

set.seed(42)
model <- word2vec(x = data_embeddings$combined_text, type = "skip-gram", dim = 80, iter = 20, stopwords = "english", window = "5")
...
lookslike %>%
  ...
  theme_minimal() +
  labs(x = "similarity score with term",
       y = "") +
  ggtitle("Top 20 similarity scores with term") +
