...
## **BERTopic** with **Custom Embeddings model** (all-mpnet-base-v2)
...
topic_model_mpnet = BERTopic(ctfidf_model=ctfidf_model, calculate_probabilities=True, verbose=True, min_topic_size=5)
...
## **BERTopic** with **Custom Embeddings model (multi-qa-mpnet-base-dot-v1)**

```{python}
vectorizer_model = CountVectorizer(ngram_range=(1, 3), stop_words="english")

# create a sentence model using the "all-mpnet-base-v2" pre-trained model 
sentence_model = SentenceTransformer("multi-qa-mpnet-base-dot-v1", device='cuda')

# encode the 'docs_marketing' text data into embeddings
# and show a progress bar while processing
embeddings_multiqa = sentence_model.encode(docs_marketing, show_progress_bar=True)

# create an instance of BERToopic with vectorizer_model and other parameters
topic_model_multiqa = BERTopic(vectorizer_model=vectorizer_model, calculate_probabilities=True, verbose=True, min_topic_size=5)

# adjust BERTopic to embeddings. It extracts topics and corresponding probabilities.
topics_multiqa, probs_multiqa = topic_model_multiqa.fit_transform(docs_marketing, embeddings_multiqa)
```
...
## **BERTopic** with **Custom Embeddings model** (bge-large-en)
...
sentence_model_bge = SentenceTransformer('BAAI/bge-large-en-v1.5', device='cuda')
embeddings_bge = sentence_model_bge.encode(docs_marketing, show_progress_bar=True, normalize_embeddings=True)

# Créez une instance de BERTopic avec votre vectorizer_model et d'autres paramètres
topic_model_bge = BERTopic(vectorizer_model=vectorizer_model, calculate_probabilities=True, verbose=True, min_topic_size=5, nr_topics=14)

# Ajustez le modèle BERTopic aux embeddings
topics_bge, probs_bge = topic_model_bge.fit_transform(docs_marketing, embeddings_bge)#model = SentenceTransformer('BAAI/bge-large-zh')
#embeddings_1 = model.encode(sentences, normalize_embeddings=True)
#embeddings_2 = model.encode(sentences, normalize_embeddings=True)
#similarity = embeddings_1 @ embeddings_2.T
#print(similarity)
...
# Ajustez le modèle BERTopic aux embeddings
topics_bge, probs_bge = topic_model_bge.fit_transform(docs_marketing, embeddings_bge)
```
...
## An analysis of BERTopics

### What is the representative document for each topic?

```{python}
topicsinfo = topic_model.get_topic_info()
topicsinfo
```
...
visualize_bertopic(topic_model_xlnet, model_name)

# bge large
model_name = "bge-large-en-v1.5"
visualize_bertopic(topic_model_bge, model_name)
...
visualize_bertopic(topic_model_multiqa, model_name)
...
fig_topics = topic_model.visualize_topics()
fig_topics.write_html("topicsinfo.html")
...
hierarchical_topics = topic_model.hierarchical_topics(docs_marketing)
fig_hierarchical_topics = topic_model.visualize_hierarchy(hierarchical_topics=hierarchical_topics)
fig_hierarchical_topics.write_html("hierarchical.html")
...
fig_topics_over_time = topic_model_mpnet.visualize_topics_over_time(topics_over_time, top_n_topics=20)
...
```{python}
topic_term_matrix = topic_model_mpnet.c_tf_idf_
