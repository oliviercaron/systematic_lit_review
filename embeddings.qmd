...
# - ngram_range=(1, 2): generate both unigrams and bigrams from the text.
...
# Étape 3 : Calcul de la distance cosinus entre les embeddings
cosine_distances = pairwise_distances(embeddings_3D, metric='cosine')

# Étape 4 : Convertir les distances cosinus en distances euclidiennes
euclidean_distances = np.sqrt(2 * (1 - cosine_distances))

# Étape 5 : Clustering avec HDBSCAN en utilisant la distance euclidienne
clusterer = hdbscan.HDBSCAN(min_cluster_size=10, metric='precomputed')
df['Cluster'] = clusterer.fit_predict(euclidean_distances)

# Étape 6 : Création d'un DataFrame pour la visualisation
df_vis = df[['X', 'Y', 'Z', 'dc_creator', 'year', 'Cluster']]

# Étape 7 : Création du graphique 3D interactif avec Plotly
fig = px.scatter_3d(df_vis, x='X', y='Y', z='Z', color='Cluster', text=df_vis.apply(lambda row: f"{row['dc_creator']}, {row['year']}", axis=1))
fig.update_traces(marker=dict(size=5))
fig.update_layout(title='Clustering 3D with HDBSCAN (Cosine Distance)')
fig.show()
fig.save
