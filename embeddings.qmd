...
## **Custom Embeddings model for BERTopic** (all-mpnet-base-v2)
...
topic_model = BERTopic(vectorizer_model=vectorizer_model, calculate_probabilities=True, verbose=True, min_topic_size=5, nr_topics=13)

# adjust BERTopic to embeddings. It extracts topics and corresponding probabilities.
topics, probs = topic_model.fit_transform(docs_marketing, embeddings_array)
```

## **Custom Embeddings model for BERTopic** (xlnet)
...
# if you want a dataframe, you can use the following commands:
...
#test similarity
query_embedding = sentence_model_bge.encode('Dogs are mammals.')
passage_embedding = sentence_model_bge.encode('Fish are aquatic animals.')
...
uery_embedding = sentence_model.encode('Dogs are mammals.')
passage_embedding = sentence_model.encode('Fish are aquatic animals.')
similarity = query_embedding @ passage_embedding
print(similarity)
print("Similarity:", util.dot_score(query_embedding, passage_embedding))
...
topic_model_bge = BERTopic(vectorizer_model=vectorizer_model, calculate_probabilities=True, verbose=True, min_topic_size=8, nr_topics=13)
...
topicsinfo_bge = topic_model_bge.get_topic_info()
topicsinfo_bge
```
...
fig_topics.write_html("topics_13.html")
...
fig_barchart.write_html("barchart_13topics.html")
```
...
fig_heatmap.write_html("fig_heatmap_13topics.html")
