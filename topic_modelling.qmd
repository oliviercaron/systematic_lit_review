...
# A lot of palettes here: https://jiffyclub.github.io/palettable/
...
# Check and create the "wordclouds" folder if it doesn't exist
output_folder = "wordclouds"
if not os.path.exists(output_folder):
    os.makedirs(output_folder)

# Loop to generate word clouds
...
```{r}
#| label: grid-wordclouds

library(gridExtra)
library(grid)
library(png)  # If you're working with PNG images

# Define the folder path where your wordcloud images are located
folder_path <- "/wordclouds"  # Replace with your actual folder path

# List all files in the folder
all_files <- list.files(folder_path)

# Define a regular expression pattern to match file names
pattern <- "^wordcloud_topic[0-9]+_GrandBudapest2_[0-9]+\\.png$"

# Use grepl to filter files that match the pattern
matching_files <- all_files[grepl(pattern, all_files)]

# Read and prepare the matching images
image_list <- lapply(matching_files, function(file) {
  image <- readPNG(file.path(folder_path, file))  # Adjust for JPEG if needed
...
grid.arrange(
  grobs = image_list,
  nrow = 1,  # Number of rows in the grid
  ncol = length(image_list)  # Number of columns based on the number of images
)
