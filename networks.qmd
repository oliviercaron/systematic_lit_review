...
list_references <- list_references %>%
  ...
  #mutate(author_year = paste0(author,", ", year)) %>% If we ever need to use this variable in the networks
...
We correct inconsistencies in the data.
Some `scopus_id` identifiers have multiple different values of title, sourcetitle, authorname, etc. (even only minor differences). 
For data consistency and future plot of networks, we need to have only one unique value for each variable of `scopus_id`.
...
```{r}
#| label: citations-get-missing-data
...
def get_citations_df(df, start_year, end_year):
    ...
    df['year'].replace('NA', np.nan, inplace=True)
    ...
    citations_df = df_filtered[['citing_art', 'scopus_id', 'sourcetitle', 'title', 'citedby-count', 'author', 'year']]
...
citations_df_overall = get_citations_df(list_references_standardized, 0, 2023)
```

### Add information about references to the dataframes

```{python}
#| label: citations-add-information

def get_info_references_dict(df, column):
    """
    Create a dictionary with keys from "scopus_id" and values from the specified column.

    :param df: Input DataFrame.
    :param column: Column name to be used as values in the resulting dictionary.
    :return: Dictionary with keys from "scopus_id" and values from the specified column.
    """
    if "scopus_id" not in df.columns or column not in df.columns:
        raise ValueError("The required columns are not present in the DataFrame.")

    return sort_dict(df.set_index("scopus_id")[column].to_dict())
  
#dict_sourcetitle_2022_2023 = get_info_references_dict(citations_df_2022_2023, "sourcetitle")

# List of columns you want to use
columns_to_extract = ['title', 'sourcetitle', 'citedby-count', 'author']
...
for period, df in dfs.items():
    ...
    for column in columns_to_extract:
        ...
        if column in df.columns:  
            dict_references[period][column] = get_info_references_dict(df, column)

#print(dict_references)  # This will print the nested dictionary
...
def sigma_graph_references(dataframe, period_label):
    ...
    Sigma.write_html(G,
        ...
        fullscreen=True,  # Display in fullscreen mode
        start_layout=5,  # Start the layout algorithm automatically and lasts 3 seconds
        ...
        #edge_color="source",  # Set edge color from 'source' attribute
        node_border_color_from='node',  # Set node border color from 'node' attribute
        node_size=G.in_degree,  # Set node size based on 'citations' attribute
        node_label_size_range=(12, 25),  # Set node label size range
...
### Citations network for 2022-2023
...
### Citations network for 2018-2021
...
### Citations network for 2013-2017
...
### Citations network for before 2013
