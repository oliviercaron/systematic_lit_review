...
def get_citations_df(df, start_year, end_year):
    ...
    # Convert the 'year' column to integer using .loc
    df['year'] = df['year'].astype(int)
    
    # Filter the data based on the 'year' column and create a distinct copy
...
columns_to_extract = ['title', 'sourcetitle', 'citedby-count', 'author', 'year']
...
def sigma_graph_references(dataframe, period_label):
    ...
    Sigma.write_html(G,
                 ...
                 start_layout           = 5                                                       # Start the layout algorithm automatically and lasts 5 seconds
...
sns.barplot(x='period', y='density', data=density_df_references)
plt.title('Density of Networks by Period')
plt.show()
```
```{r}
...
density_plot <- ggplot() +
  geom_line(data = py$density_df, aes(x = period, y = density, colour = "Collaboration Density", group=1), size = 1) +
  geom_line(data = py$density_df_references %>% filter(period != "overall"), aes(x = period, y = density, colour = "References Density", group=1), size = 1) +
  scale_y_continuous(name = "Collaboration Density", sec.axis = sec_axis(~ . / 10, name = "References Density")) +
...
density_plot <- ggplot() +
  geom_bar(data = py$density_df, aes(x = period, y = density, fill = "Collaboration Density"), stat="identity", position="dodge") +
  geom_bar(data = py$density_df_references %>% filter(period != "overall"), aes(x = period, y = density, fill = "References Density"), stat="identity", position="dodge") +
  scale_y_continuous(name = "Collaboration Density", sec.axis = sec_axis(~ . / 10, name = "References Density")) +
  scale_x_discrete(limits = c("before-2013", "2013-2017", "2018-2021", "2022-2023")) +
  xlab("Period") +
  ggtitle("Comparison of Network Density Over Time") +
